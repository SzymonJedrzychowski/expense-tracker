...
public class ExpenseController {
    ...
    public Expense createExpense(@RequestBody @Valid CreateExpenseRequestDTO createExpenseRequestDTO) {
        ...
        Account account = accountRepository.findById(createExpenseRequestDTO.getAccountId())
                .orElseThrow(() -> new ReasonedResponseStatusException(
                        HttpStatus.NOT_FOUND,
        ...
        ExpenseType expenseType = expenseTypeRepository.findById(createExpenseRequestDTO.getExpenseTypeId())
                .orElseThrow(() -> new ReasonedResponseStatusException(
                        HttpStatus.NOT_FOUND,
                        String.format("Cannot find Expense Type with ID: %d", createExpenseRequestDTO.getExpenseTypeId())
                ));
        ...
        LocalDate date = createExpenseRequestDTO.getDate();
        float negativeMovement = -Math.min(createExpenseRequestDTO.getMovementAmount(), 0);
        float positiveMovement = Math.max(createExpenseRequestDTO.getMovementAmount(), 0);
        ...
        //Update the values
        accountState.setCurrentAmount(accountState.getCurrentAmount() + createExpenseRequestDTO.getMovementAmount());
    ...
    public Expense updateExpense(@RequestBody @Valid UpdateExpenseRequestDTO updateExpenseRequestDTO) {
        ...
        Expense expenseToUpdate = expenseRepository.findById(updateExpenseRequestDTO.getId())
                .orElseThrow(() -> new ReasonedResponseStatusException(
                        HttpStatus.NOT_FOUND,
                        String.format("Cannot find Expense with ID %d.", updateExpenseRequestDTO.getId())
                ));
        ...
        if (Objects.equals(updateExpenseRequestDTO.getExpenseTypeId(), expenseType.getId())) {
            expenseType = expenseTypeRepository.findById(updateExpenseRequestDTO.getExpenseTypeId())
                    .orElseThrow(() -> new ReasonedResponseStatusException(
                            HttpStatus.NOT_FOUND,
                            String.format("Cannot find Expense Type with ID: %d",
                                    updateExpenseRequestDTO.getExpenseTypeId())
                    ));
        }
        ...
        LocalDate date = updateExpenseRequestDTO.getDate();
